...
$C$-true stage $s$, $hat(Phi)_(s)(C_s; x) arrow.r.double hat(Phi)(C; x) converge$. The hat-trick serves to weed out at $C$-true stages
...
#proof[
    ...
    cycle $(j, k')$ would be reset. We may may immediately discount the possibility that $(j, k'')$ is in state~7,
...
#theorem(name: [Jockusch and Shore])[
    ...
    such that for all $i geq 0$, $F notequivt A_i$.
    <theorem3.3>
]
...
#theorem[
    If $C ltt H$ are r.e. and high (that is, $C' equivt H' equivt emptyset''$), there is a d.r.e. set
...
+ Denote by $Eq(x, s)$ the condition
  $
  and.big_(i=0,1) [(restr(A_i, x + 1))[s] = (restr(Phi_(e,i)(E_(e,i)), x+1))[s] and #h(1fr) \
...
Consider the requirement $P_(2e)$. ($P_(2e+1)$ is the same, with $A_0$ and $B_0$ replaced with $A_1$ and $B_1$.)
...
#proof[
    ...
    + Strategy $alpha$ is cancelled by some cycle $gamma subset.neq alpha$ or $gamma ltl alpha$ seeing a $C$-change
      ...
      If $gamma ltl alpha$ then let $delta$ be the longest common initial segment of $alpha$ and~$gamma$.
...
#proof[
    ...
    For (iii), if $restr(C, macron(v)(alpha, l))$ changes after #stg($s(l)$) then $alpha$'s #cycle($l$) would
